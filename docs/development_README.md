...
- [ ] Start the game and view score
- [ ] Answer questions and select hints
- [ ] Use easel.js to somehow reveal hints
...
`board.js`: setting up each round and maintaining main Easel.js 'stage'

`timer.js`: this script will handle the scoring logic behind the scenes, rendering players' score and time.
...
- Build in keyboard action rather than just cursor action
  ...
  **Day 3**: Create the `game.js` logic backend.  Build out logic for devising score based on time spent playing per question or on how many hints were viewed. Incorporate the logic into the rendering.  Goals for the day:
